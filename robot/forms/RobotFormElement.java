...
import robot.robotApp.RobotConfig;

/**
 * FormElementComponent subclass that allows a user to specify basic/common 
 * Robot attribute values, e.g. width, height etc. The value are held in the 
 * form and used by the Form's associated FormHandler to construct an instance
 * of a specified Robot subclass.
 * 
 * A RobotFormElement is a GUI component that has sliders for selecting a 
 * Robot's width, height, delta-x and delta-y values. A text field is used to
 * enter any text to be associated with a new Robot.
 * 
...
public class RobotFormElement extends FormElementComponent {
	...
	private static final int MAX_WIDTH = RobotConfig.instance().getAnimationBounds().width / 2;
	private static final int MIN_HEIGHT = 10;
	private static final int MAX_HEIGHT = RobotConfig.instance().getAnimationBounds().height / 2;
	...
	/**
	 * Creates a RobotFormElement with fields to store Robot attribute values.
	 */
	public RobotFormElement() {
		...
		setBorder(new CompoundBorder(BorderFactory.createTitledBorder("Basic robot properties"), border));
